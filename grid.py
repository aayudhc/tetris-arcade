...
class Grid:
    ...
    def draw_grid_lines(self) -> None:
        ...
        for y in range(1, GRID_HEIGHT):
            arcade.draw_line(
                0, y * CELL_SIZE, GRID_H_SIZE, y * CELL_SIZE, GRID_LINE_COLOUR
            )

        # vertical lines
        for x in range(1, GRID_WIDTH):
            arcade.draw_line(
                x * CELL_SIZE, 0, x * CELL_SIZE, GRID_V_SIZE, GRID_LINE_COLOUR
            )

    def get_cell(
        self, x0: int, x1: int
    ) -> int:  # x0 & x1 are the 2 dimensions of matrix
        return self.main_grid[x0][x1]

    def set_cell(
        self, x0: int, x1: int, val: int
    ) -> None:  # x0 & x1 are the 2 dimensions of matrix
        ...
        for x in range(GRID_HEIGHT):
            for y in range(GRID_WIDTH):
                ...
                arcade.draw_lbwh_rectangle_outline(
                    square_x, square_y, CELL_SIZE, CELL_SIZE, BOX_MARGIN_COLOUR
                )
                arcade.draw_lbwh_rectangle_filled(
                    square_x, square_y, CELL_SIZE, CELL_SIZE, BORDERING_BOX_COLOUR
                )
    ...
    def can_rotate_letter(self) -> bool:
        ...
        pattern_next = self.active_letter[
            (self.letter_rotation + 1) % self.active_letter["rotation_count"]
        ]
        ...
        for x in range(len(pattern_next)):
            for y in range(len(pattern_next[0])):
                if x < len(pattern_current) and y < len(pattern_current[0]):
                    if (
                        pattern_current[x][y] != 0
                    ):  # skips check if place curruntly occupied by old block
                        continue
    ...
    def rotate_letter(self) -> None:
        ...
        self.letter_rotation = (self.letter_rotation + 1) % self.active_letter[
            "rotation_count"
        ]
    ...
    def _can_move_left(self) -> bool:
        ...
        for x in range(current_x, current_x + len(pattern)):
            test_grid.append(
                self.main_grid[x][current_y - 1 : current_y + len(pattern[0])]
            )
        ...
        for x in range(0, len(pattern)):
            for y in range(0, len(pattern[0])):
                if pattern[x][y] == 0:  # if block is empty, no need to check
                    continue
                if (
                    y - 1 >= 0 and pattern[x][y - 1] != 0
                ):  # if the block its moving to was occupied by the same pattern, no need to check
                    continue
    ...
    def _can_move_right(self) -> bool:
        ...
        for x in range(current_x, current_x + len(pattern)):
            test_grid.append(
                self.main_grid[x][current_y : current_y + len(pattern[0]) + 1]
            )
        ...
        for y in range(len(test_grid[0]) - 2, -1, -1):
            for x in range(len(test_grid)):
                ...
                if (
                    test_grid[x][y + 1] != 0
                ):  # if letter can't be accepted in new position
                    return False
        ...
        for x in range(0, len(pattern)):
            for y in range(0, len(pattern[0])):
                if pattern[x][y] == 0:  # if block is empty, no need to check
                    continue
                if (
                    y + 1 < len(pattern[0]) and pattern[x][y + 1] != 0
                ):  # if the block its moving to was occupied by the same pattern, no need to check
                    continue
    ...
    def can_move_down(self) -> bool:
        ...
        for x in range(current_x + len(pattern) - 1, current_x - 1, -1):
            for y in range(current_y, current_y + len(pattern[0])):
                if (
                    y in accounted_cols
                ):  # only check lowermost filled block in each column
                    continue

                if (
                    pattern[x - current_x][y - current_y] == 0
    ...
    def draw_preview_box(self) -> None:
        arcade.draw_lbwh_rectangle_outline(
            PREVIEW_Y * CELL_SIZE - 1,
            PREVIEW_X * CELL_SIZE - 1,
            3 * CELL_SIZE + 1,
            4 * CELL_SIZE + 1,
            GRID_LINE_COLOUR,
        )
